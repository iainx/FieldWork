...
@implementation MLNLoadOperation {
    ...
    NSError *_error;
    void (^_completionHandler)(uint64, AudioStreamBasicDescription, NSError *);
}
...
+ (MLNLoadOperation *)createLoadOperationFromURLOnMainQueue:(NSURL *)url
                                               metadataOnly:(BOOL)metadataOnly
                                                     action:(nullable void (^)(uint64, AudioStreamBasicDescription, NSError *))completionBlock
{
    MLNLoadOperation *_currentOperation = [[MLNLoadOperation alloc] initFromURL:url metadataOnly:metadataOnly];
    NSOperationQueue *defaultQueue = [MLNLoadOperation defaultOperationQueue];
    
    _currentOperation->_completionHandler = completionBlock;
    [_currentOperation setDelegate:nil]; // FIXME: Do we need a delegate here?
    [defaultQueue addOperation:_currentOperation];
    
    return _currentOperation;
}

- (instancetype)initFromURL:(NSURL *)url metadataOnly:(BOOL)metadataOnly
{
    self = [super init];
    if (!self) {
        return nil;
    ...
    if (check_status_is_error(status, "ExtAudioFileOpenURL")) {
        ...
        if (_error) {
            ...
            if (_completionHandler) {
                _completionHandler(0, _outputFormat, _error);
            }
        }
...
- (void)updateSampleOnMainThread:(id)object
{
    ...
    if (_completionHandler) {
        _completionHandler(_totalFrameCount, _outputFormat, nil);
    }
