...
- (id)initWithChannels:(NSArray *)channelData
{
    ...
    // Not a deep copy
    channelData = [channelData mutableCopy];
...
+ (MLNSample *)previewSample
{
    ...
    float *data = malloc(sizeof(float) * 441000 * 2);
    for (int i = 0; i < 441000 * 2; i++) {
        data[i] = sinf(((float)i / 57.29578) / 256.0);
    }
    [channel addData:data withByteLength:441000 * 2];
    
    MLNSample *sample = [[MLNSample alloc] initWithChannels:channelData];
    return sample;
}
...
- (void)startImportFromURL:(NSURL *)url
           progressHandler:(nullable void (^)(uint64, uint64))importProgressBlock
{
    _url = url;
    
    [self importFromURL:url progressHandler:importProgressBlock];
}

- (void)importFromURL:(NSURL *)url
      progressHandler:(nullable void (^)(uint64, uint64))importProgressBlock
{
    _currentOperation = [[MLNLoadOperation alloc] initFromURL:url metadataOnly:NO];
    NSOperationQueue *defaultQueue = [MLNOperation defaultOperationQueue];
    
    [_currentOperation setDelegate:self];
    [_currentOperation setProgressHandler:importProgressBlock];
             ...
             asFormat:(NSDictionary *)format
{
    ...
    NSOperationQueue *defaultQueue = [MLNLoadOperation defaultOperationQueue];
